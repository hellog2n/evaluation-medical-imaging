...
class InceptionV3(nn.Module):
    ...
    def __init__(self,
        ...
        inception = _inception_v3(pretrained=self.pretrained)
        if self.finetuned:

            finetuned_path = kwargs['finetuned_path']
            inception.fc = nn.Linear(2048, 2)
            inception.AuxLogits.fc = nn.Linear(768, 2)
            inception.load_state_dict(torch.load(finetuned_path), strict=True)
